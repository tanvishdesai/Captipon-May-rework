...
MODEL_SAVE_PATH = "./best_transformer_caption_model_augmented.pth" # Changed model name
...
class FlickrDataset(Dataset):
    def __init__(self, root_dir, captions_dict, image_names, vocab, transform, max_len): # transform is passed here
        ...
        self.transform = transform # This will be either train_transform or val_transform
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            image = self.transform(dummy_image) # Apply the passed transform
        ...
        image = self.transform(image) # Apply the passed transform
...
def plot_metrics(train_losses, val_losses, bleu_scores_history):
    ...
    plt.legend(); plt.grid(True)
    plt.tight_layout(); plt.savefig("training_metrics_transformer_augmented.png"); plt.show()
    print("Metrics plot saved as training_metrics_transformer_augmented.png")
...
def main():
    ...
    # --- MODIFIED SECTION: Define train and validation transforms ---
    # Training transforms with augmentations
    train_transform = transforms.Compose([
        transforms.Resize(288), # Resize slightly larger before random crop
    ...
    print("Defined training and validation image transforms with augmentations for training.")
    ...
    # --- MODIFIED SECTION: Pass appropriate transforms to datasets ---
    ...
    for epoch in range(num_epochs):
        ...
        # Evaluate every epoch or less frequently (e.g., every 5 epochs as before)
        # For faster iteration during development, you might evaluate more often.
        # if ((epoch+1) % 1 == 0) or epoch == num_epochs -1 : # Evaluate every epoch
        if ((epoch+1) % 5 == 0) or epoch == num_epochs -1 : # Or keep original evaluation frequency
    ...
    # --- 6.5 Plot Metrics ---
    if train_losses and val_losses and bleu_scores_history: # bleu_scores_history might be shorter if not eval every epoch
        print("\n--- 6.5 Plotting Metrics ---")
        # Adjust plotting if validation is not done every epoch for val_losses and bleu_scores_history
        # This current plot_metrics assumes len(train_losses) == len(val_losses) == len(bleu_scores_history)
        # If not, you'll need to adjust the x-axis for val_losses and bleu_scores_history.
        # For simplicity, if keeping eval every 5 epochs, this part might need adjustment or ensure arrays align.
        # One way is to plot train_losses against epochs 1..N, and val_losses/bleu against epochs k, 2k, 3k...
        
        # Simplification: Ensure plot_metrics can handle potentially different lengths or only plot if eval was frequent.
        # If evaluating less frequently, the arrays might not align.
        # For now, assume plot_metrics is called only if they are populated sufficiently.
        # If val_losses and bleu_scores_history are shorter (due to less frequent eval), adjust the plot_metrics function
        # or the data passed to it.
        # Example:
        # plot_epochs = range(1, len(train_losses) + 1)
        # val_epochs = [e for e in plot_epochs if (e % 5 == 0) or e == len(train_losses)] # if eval every 5 epochs
        # For now, the original plot_metrics is called, it might error if lengths don't match.
        # Let's assume for now that if bleu_scores_history is populated, val_losses is too.
        
        # To make plot_metrics robust to less frequent validation:
        epochs_range_train = range(1, len(train_losses) + 1)
        epochs_range_val = [ (i+1)*5 for i in range(len(val_losses)) ] # Assuming eval every 5 epochs
        if num_epochs % 5 != 0 and (num_epochs-1) not in epochs_range_val and len(val_losses) > 0: # If last epoch was eval
            if len(epochs_range_val) < len(val_losses): # if last epoch eval added an extra val point
                 epochs_range_val.append(num_epochs)
        ...
        if val_losses:
            # Ensure epochs_range_val matches length of val_losses
            val_plot_epochs = epochs_range_val[:len(val_losses)]
            plt.plot(val_plot_epochs, val_losses, 'ro-', label='Validation Loss')
        plt.title('Training and Validation Loss')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.legend(); plt.grid(True)
        ...
        plt.subplot(1, 2, 2)
        if bleu_scores_history:
            bleu_plot_epochs = epochs_range_val[:len(bleu_scores_history)]
            for i in range(1, 5):
                bleu_i_scores = [scores[f'BLEU-{i}'] for scores in bleu_scores_history]
                if bleu_i_scores:
                    plt.plot(bleu_plot_epochs, bleu_i_scores, marker='o', linestyle='-', label=f'BLEU-{i}')
        plt.title('Validation BLEU Scores'); plt.xlabel('Epochs'); plt.ylabel('BLEU Score')
        plt.legend(); plt.grid(True)
        plt.tight_layout(); plt.savefig("training_metrics_transformer_augmented.png"); plt.show()
        print("Metrics plot saved as training_metrics_transformer_augmented.png")
        ...
        print("\n--- 6.5 Skipping Plotting: No/Insufficient metrics recorded. ---")
    ...
    if os.path.exists(MODEL_SAVE_PATH) and len(val_dataset) > 0:
        ...
        if all(key.startswith('module.') for key in state_dict.keys()) and not isinstance(final_model, nn.DataParallel):
            ...
            final_model.load__state_dict(state_dict)
        
        final_model.eval()
        
        # --- MODIFIED SECTION: Get sample image using val_transform for consistency in generation ---
        # The val_dataset already uses val_transform, so this is fine.
        sample_idx = random.randint(0, len(val_dataset)-1)
        sample_img_tensor_from_dataset, sample_caption_tensor, _ = val_dataset[sample_idx]
        
        # If you wanted to load a raw image and preprocess it for inference:
        # img_name, _ = val_dataset.df[sample_idx] # Get an image name from validation set
        # img_path = os.path.join(IMAGE_DIR, img_name)
        # raw_image = Image.open(img_path).convert("RGB")
        # sample_img_tensor_manual = val_transform(raw_image) # Apply val_transform
        
        # For this example, we use the image tensor directly from the val_dataset item
        sample_img = sample_img_tensor_from_dataset 
