...
import nltk # Already imported but good to ensure it's here for word_tokenize
...
CONFIG = {
    ...
    "spm_model_prefix": "type4_gujarati_spm", # Updated for Type-4
    "spm_vocab_size": 8000,
...
def calculate_all_metrics(references_dict, candidates_dict, sp_processor):
    actual_references_bleu_tokenized_spm = []
    predicted_candidates_bleu_tokenized_spm = []
    
    meteor_scores_list = []
    rouge_l_fmeasure_list = []
    ...
    for img_key, pred_text in candidates_dict.items():
        if img_key in references_dict:
            ...
            if not refs_for_image_text:  # Should not happen with valid data
                continue

            # BLEU: Use SentencePiece subword tokenization (as originally)
            tokenized_refs_spm = [sp_processor.encode_as_pieces(ref) for ref in refs_for_image_text]
            tokenized_pred_spm = sp_processor.encode_as_pieces(pred_text)
            actual_references_bleu_tokenized_spm.append(tokenized_refs_spm)
            predicted_candidates_bleu_tokenized_spm.append(tokenized_pred_spm)

            # METEOR: Use NLTK word tokenization
            # NLTK's meteor_score expects list of reference lists, but here we have one list of refs for one hypo
            # So, we pass the list of tokenized reference strings directly.
            tokenized_refs_nltk = [nltk.word_tokenize(ref) for ref in refs_for_image_text]
            tokenized_pred_nltk = nltk.word_tokenize(pred_text)
            if tokenized_pred_nltk: # meteor_score can fail on empty hypothesis
                 meteor_val = meteor_score(tokenized_refs_nltk, tokenized_pred_nltk)
                 meteor_scores_list.append(meteor_val)

            # ROUGE-L: Use raw strings, scorer handles tokenization
            current_rouge_l_scores = []
            for ref_text in refs_for_image_text:
                if ref_text and pred_text: # Ensure neither is empty
                    score = rouge_scorer_instance.score(ref_text, pred_text)
                    current_rouge_l_scores.append(score['rougeL'].fmeasure)
            if current_rouge_l_scores:
                rouge_l_fmeasure_list.append(np.mean(current_rouge_l_scores))

    # Calculate BLEU scores
    bleu1, bleu2, bleu3, bleu4 = 0.0, 0.0, 0.0, 0.0
    if predicted_candidates_bleu_tokenized_spm:
        chencherry = SmoothingFunction()
        bleu1 = corpus_bleu(actual_references_bleu_tokenized_spm, predicted_candidates_bleu_tokenized_spm, weights=(1.0, 0, 0, 0), smoothing_function=chencherry.method1)
        bleu2 = corpus_bleu(actual_references_bleu_tokenized_spm, predicted_candidates_bleu_tokenized_spm, weights=(0.5, 0.5, 0, 0), smoothing_function=chencherry.method1)
        bleu3 = corpus_bleu(actual_references_bleu_tokenized_spm, predicted_candidates_bleu_tokenized_spm, weights=(0.33, 0.33, 0.33, 0), smoothing_function=chencherry.method1)
        bleu4 = corpus_bleu(actual_references_bleu_tokenized_spm, predicted_candidates_bleu_tokenized_spm, weights=(0.25, 0.25, 0.25, 0.25), smoothing_function=chencherry.method1)
    
    avg_meteor_score = np.mean(meteor_scores_list) if meteor_scores_list else 0.0
    avg_rouge_l_fmeasure = np.mean(rouge_l_fmeasure_list) if rouge_l_fmeasure_list else 0.0
    
    return bleu1, bleu2, bleu3, bleu4, avg_meteor_score, avg_rouge_l_fmeasure
...
def evaluate(model, dataloader, criterion, device, epoch, sp_processor, all_captions_master_dict):
    ...
    bleu1, bleu2, bleu3, bleu4, meteor_score_val, rouge_l_fmeasure_val = calculate_all_metrics(
        all_captions_master_dict, candidate_captions_dict, sp_processor
    )
    return avg_loss, bleu1, bleu2, bleu3, bleu4, meteor_score_val, rouge_l_fmeasure_val
...
def main():
    ...
    best_bleu4_model_path = "best_type4_caption_model.pth" # Updated for Type-4
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        # Evaluate now returns more metrics
        val_loss, bleu1, bleu2, bleu3, bleu4, meteor_score_val, rouge_l_fmeasure_val = evaluate(
            caption_model, val_loader, criterion, CONFIG['device'], epoch, sp_processor, all_captions_dict
        )
        ...
        print(f"  Other Metrics: METEOR={meteor_score_val:.4f}, ROUGE-L={rouge_l_fmeasure_val:.4f}") # New metrics
        if CONFIG['device'].type == 'cuda':
        ...
        history['meteor'].append(meteor_score_val) # Store new metric
        history['rougeL'].append(rouge_l_fmeasure_val) # Store new metric
    ...
    if not history['train_loss']: 
        ...
        # Adjust figure size if adding more subplots, e.g., figsize=(18, 16) for 3 rows
        plt.figure(figsize=(18, 16)) 

        plt.subplot(3, 2, 1) # Changed from 2,2,1
        ...
        plt.subplot(3, 2, 2) # Changed from 2,2,2
        ...
        # New subplot for METEOR and ROUGE-L
        plt.subplot(3, 2, 3) 
        if history['meteor']:
            plt.plot(epochs_ran, history['meteor'], label='METEOR', marker='.', color='orange')
        if history['rougeL']:
            plt.plot(epochs_ran, history['rougeL'], label='ROUGE-L (F1)', marker='.', color='red')
        plt.xlabel('Epochs'); plt.ylabel('Score'); plt.legend(); plt.title('METEOR & ROUGE-L Scores')
        plt.grid(True)

        if history['epoch_duration']:
            plt.subplot(3, 2, 4) # Adjusted index
        ...
        if CONFIG['device'].type == 'cuda' and any(m > 0 for m in history['gpu_mem_peak_epoch']):
            ...
            if gpu_mems_to_plot:
                plt.subplot(3, 2, 5) # Adjusted index
        ...
        # Potentially add a 6th plot or leave empty if 3,2 layout
        # plt.subplot(3, 2, 6)
        # plt.axis('off') # Example: turn off axis for an empty subplot


        plt.tight_layout()
        plt.savefig("type4_training_and_resource_plots.png") # Updated for Type-4
        print("Plots saved to type4_training_and_resource_plots.png") # Updated for Type-4
    ...
    if os.path.exists(best_bleu4_model_path):
        generate_output_csv(
            ...
            config=CONFIG,
            output_csv_path="type4_all_images_captions_output.csv" # Updated for Type-4
        )
    else:
        print(f"Skipping CSV generation as best model file '{best_bleu4_model_path}' was not found.")

    print("\n--- Script Finished Type-4 ---") # Updated comment
