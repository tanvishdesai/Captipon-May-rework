...
num_layers_decoder = 1  # Number of layers in LSTM decoder
dropout_p = 0.5
...
class ImageCaptioningModel(nn.Module):
    ...
    def generate_caption(self, image, vocab, max_sample_length=20):
        ...
        with torch.no_grad():
            ...
            sampled_ids = self.decoder.sample(encoder_out, vocab, max_sample_length)
    ...
    with torch.no_grad():
        for images, captions_batch, _ in tqdm(val_loader, desc=f"Epoch {epoch+1}/{num_epochs} [Validation]", unit="batch"):
            ...
            for i in range(images.size(0)):
                ...
                # Generated caption
                generated_caption_text = model.module.generate_caption(image_single, vocab, max_caption_length) \
                                         if isinstance(model, nn.DataParallel) else \
                                         model.generate_caption(image_single, vocab, max_caption_length)
...
def main():
    ...
    if os.path.exists(MODEL_SAVE_PATH) and len(val_dataset) > 0 :
        ...
        # Generated caption
        generated_caption = final_model.module.generate_caption(sample_img, vocab) \
                            if isinstance(final_model, nn.DataParallel) else \
                            final_model.generate_caption(sample_img, vocab)
